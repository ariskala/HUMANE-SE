...
output_file_path = "mapped_skills_with_edu_policies.json"
...
print("Mapping skills through the SKILLAB skills API...")
...
for index, skill in enumerate(unique_skills, start=1):
    ...
    try:
        ...
        if (
            ...
            or not isinstance(response_data["items"], list)
        ):
        ...
        item_data = response_data["items"][0] if response_data["items"] else {}

        skill_mappings[skill] = {
            "skill_label_from_mapping": item_data.get("label", "Unknown"),
            "alternative_labels": item_data.get("alternative_labels", [])
        }
...
print("Adding mapped skills to job postings...")
